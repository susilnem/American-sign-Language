...
def draw_skeleton(canvas, pts, os_x, os_y):
    """Draw hand skeleton onto a canvas given 21 landmark points and an (x, y) offset."""
    finger_segments = [(0, 4), (5, 8), (9, 12), (13, 16), (17, 20)]
    for start, end in finger_segments:
        for t in range(start, end):
            cv2.line(canvas, (pts[t][0]+os_x, pts[t][1]+os_y),
                     (pts[t+1][0]+os_x, pts[t+1][1]+os_y), (0, 255, 0), 3)
    for a, b in [(5, 9), (9, 13), (13, 17), (0, 5), (0, 17)]:
        cv2.line(canvas, (pts[a][0]+os_x, pts[a][1]+os_y),
                 (pts[b][0]+os_x, pts[b][1]+os_y), (0, 255, 0), 3)
    for i in range(21):
...
def predict_letter(pts, prob):
    ...
    if pl in [[5,2],[5,3],[3,5],[3,6],[3,0],[3,2],[6,4],[6,1],[6,2],[6,6],[6,7],[6,0],[6,5],
    ...
    if pl in [[0,0],[0,6],[0,2],[0,5],[0,1],[0,7],[5,2],[7,6],[7,1]]:
        if (pts[0][0] > pts[8][0] and pts[0][0] > pts[4][0] and pts[0][0] > pts[12][0]
                and pts[0][0] > pts[16][0] and pts[0][0] > pts[20][0]) and pts[5][0] > pts[4][0]:
            ch1 = 2
    ...
    if pl in [[1,4],[1,5],[1,6],[1,3],[1,0]]:
        if (pts[6][1] > pts[8][1] and pts[14][1] < pts[16][1] and pts[18][1] < pts[20][1]
                and pts[0][0] < pts[8][0] and pts[0][0] < pts[12][0]
    ...
    if pl in [[1,4],[1,6],[1,1]]:
        if (distance(pts[4], pts[11]) > 50
                and pts[6][1] > pts[8][1] and pts[10][1] < pts[12][1]
    ...
    if pl in [[3,2],[3,1],[3,6]]:
        if (pts[4][1]+17 > pts[8][1] and pts[4][1]+17 > pts[12][1]
                and pts[4][1]+17 > pts[16][1] and pts[4][1]+17 > pts[20][1]):
            ch1 = 5
    ...
    if (pts[0][0] > pts[8][0] and pts[0][0] > pts[12][0]
            and pts[0][0] > pts[16][0] and pts[0][0] > pts[20][0]
            and pts[4][1] < pts[8][1] and pts[4][1] < pts[12][1]
            and pts[4][1] < pts[16][1] and pts[4][1] < pts[20][1]
            and pts[4][1] < pts[6][1] and pts[4][1] < pts[10][1]
